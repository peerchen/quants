#include "thosttradehandler.h"

CThostTraderHandler::CThostTraderHandler()
{
	m_pTraderApi = NULL;
	m_pLogHandler = NULL;
}

CThostTraderHandler::CThostTraderHandler(string InvestorId, string Password, string BrokerId, string FrontAddress, string FlowPath, string UserProductInfo) :
...
int CThostTraderHandler::Start(string JsonParam)
{
	ReqConnectFront();
	return 0;
}

int CThostTraderHandler::Wait()
{
	m_pTraderApi->Join();
	return 0;
}

void CThostTraderHandler::Stop()
{
	m_pTraderApi->Release();
}

int CThostTraderHandler::ReqConnectFront()
{
	string sflowpath;
	if (m_sUserDataPath != "")
	{
		sflowpath = m_sUserDataPath + "/flow/ctp/trade/" + m_sInvestorId + "/";
	}
	else
	{
		string sflowpath = "./flow/ctp/trade/" + m_sInvestorId + "/";
	}
...
int CThostTraderHandler::ReqUserLogin(char ReqType)
{
	CThostFtdcReqUserLoginField reqUserLogin;
	memset(&reqUserLogin, 0, sizeof(CThostFtdcReqUserLoginField));
	strcpy(reqUserLogin.BrokerID,m_sBrokerId.c_str());
	strcpy(reqUserLogin.UserID, m_sInvestorId.c_str());
	strcpy(reqUserLogin.Password, m_sPasswd.c_str());
	int nRet;
	while (true)
	{
		nRet = m_pTraderApi->ReqUserLogin(&reqUserLogin, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
...
int CThostTraderHandler::ReqQrySettlementInfo(char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQrySettlementInfo(&qrySettlementInfo, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQrySettlementInfoConfirm(char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQrySettlementInfoConfirm(&qrySettlementInfoConfirm, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryInstrument(char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryInstrument(&qryInstrument, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryInstrumentMarginRate(string InstrumentID, char HedgeFlag, char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryInstrumentMarginRate(&qryInstrumentMarginRate, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryInstrumentCommissionRate(string InstrumentID, char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryInstrumentCommissionRate(&qryInstrumentCommissionRate, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryTradingAccount(char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryTradingAccount(&qryTradingAccount, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryInvestorPosition(string InstrumentID, char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryInvestorPosition(&qryInvestorPosition, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryOrder(string InstrumentID, string ExchangeID, string OrderSysID, string InsertTimeStart, string InsertTimeEnd, char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryOrder(&qryOrder, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
int CThostTraderHandler::ReqQryTrade(string InstrumentID, string ExchangeID, string TradeID, string TradeTimeStart, string TradeTimeEnd, char ReqType)
{
	...
	while (true)
	{
		nRet = m_pTraderApi->ReqQryTrade(&qryTrade, ++m_nRequestId);
		if (!IsFlowControl(nRet))
		{
			m_mapReqType[m_nRequestId] = ReqType;
			break;
...
void CThostTraderHandler::MakeOrderInsertBase(CThostFtdcInputOrderField &reqInputOrderField, string InstrumentID, char Direction, char OffsetFlag, double LimitPrice, int Volume, char HedgeFlag /* = THOST_HF_Speculation */)
{
...
void CThostTraderHandler::LimitOrder(int RequestID, string InstrumentID, char Direction, char OffsetFlag, double LimitPrice, int Volume, char HedgeFlag /* = THOST_HF_Speculation */)
{
...
void CThostTraderHandler::MarketOrder(int RequestID, string InstrumentID, char Direction, char OffsetFlag, int Volume, char HedgeFlag /* = THOST_HF_Speculation */)
{
	CThostFtdcInputOrderField reqInputOrderField;
...
void CThostTraderHandler::FakOrder(int RequestID, string InstrumentID, char Direction, char OffsetFlag, double LimitPrice, int Volume, char HedgeFlag /* = THOST_HF_Speculation */)
{
	CThostFtdcInputOrderField reqInputOrderField;
	MakeOrderInsertBase(reqInputOrderField, InstrumentID, Direction, OffsetFlag, LimitPrice, Volume, HedgeFlag);
	reqInputOrderField.RequestID = RequestID;
...
void CThostTraderHandler::FokOrder(int RequestID, string InstrumentID, char Direction, char OffsetFlag, double LimitPrice, int Volume, char HedgeFlag /* = THOST_HF_Speculation */)
{
	CThostFtdcInputOrderField reqInputOrderField;
	MakeOrderInsertBase(reqInputOrderField, InstrumentID, Direction, OffsetFlag, LimitPrice, Volume, HedgeFlag);
	reqInputOrderField.RequestID = RequestID;
...
string CThostTraderHandler::ReqOrderInsert(CThostFtdcInputOrderField &reqInputOrderField)
{
	TThostFtdcOrderRefType OrderRef;
	sprintf(OrderRef, "%12d", ++m_nOrderRef);
...
int CThostTraderHandler::CancelOrder(string OrderRef, string OrderSysID, string ExchangeID)
{
...
void CThostTraderHandler::OnFrontConnected()
{
	boost::mutex::scoped_lock lock(m_SyncReqMutex);
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Info("CTPTD:OnFrontConnected");
	}
	ClearTradingData();
	ReqUserLogin(THOST_RT_ASYNC_INIT);
}

void CThostTraderHandler::OnFrontDisconnected(int nReason)
{
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Info("CTPTD:OnFrontDisconnected:%d", nReason);
	}
}

void CThostTraderHandler::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (bIsLast)
	{
		...
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			...
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:login success!");
			}
		...
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:login failed. Reason:%s!", pRspInfo->ErrorMsg);
			}
			///前置机不活跃或还没有初始化, 睡眠10秒再重试
			if (pRspInfo->ErrorID == 8 || pRspInfo->ErrorID == 7)
			{
				boost::this_thread::sleep(boost::posix_time::milliseconds(THOST_SYNC_RQ_TIMEOUT_S*1000));
				if (m_pLogHandler != NULL)
				{
					m_pLogHandler->Info("CTPTD:ready to relogin.");
				}
...
void CThostTraderHandler::OnRspQrySettlementInfo(CThostFtdcSettlementInfoField *pSettlementInfo, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query settlement success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query settlement failed.Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
		...
		{
			ReqQrySettlementInfoConfirm(THOST_RT_ASYNC_INIT);
		}
	}
}

void CThostTraderHandler::OnRspQrySettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:confirm settlement sucess!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:confirm settlemenet failed.Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
...
void CThostTraderHandler::OnRspQryInstrument(CThostFtdcInstrumentField *pInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query instrument success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query instrument failed. Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
...
void CThostTraderHandler::OnRspQryInstrumentMarginRate(CThostFtdcInstrumentMarginRateField *pInstrumentMarginRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
...
void CThostTraderHandler::OnRspQryInstrumentCommissionRate(CThostFtdcInstrumentCommissionRateField *pInstrumentCommissionRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (pInstrumentCommissionRate != NULL)
	{
		...
		for (InstrumentInfoMap::iterator iter = m_mapInstrumentInfo.begin(); iter != m_mapInstrumentInfo.end(); ++iter)
		{
			...
			{
				instrumentCommissionRateField.InstrumentID = pInstrumentCommissionRate->InstrumentID;
...
void CThostTraderHandler::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pTradingAccount, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query banlance success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query balance failed. Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
...
void CThostTraderHandler::OnRspQryInvestorPosition(CThostFtdcInvestorPositionField *pInvestorPosition, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query position success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query position failed. Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
...
void CThostTraderHandler::OnRspQryInvestorPositionDetail(CThostFtdcInvestorPositionDetailField *pInvestorPositionDetail, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (bIsLast)
	{
		char cReqType = THOST_RT_ASYNC;
...
void CThostTraderHandler::OnRspQryOrder(CThostFtdcOrderField *pOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query order success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query order failed. Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
...
void CThostTraderHandler::OnRspQryTrade(CThostFtdcTradeField *pTrade, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (pTrade != NULL)
	{
	...
	if (bIsLast)
	{
		if (pRspInfo == NULL || (pRspInfo != NULL && pRspInfo->ErrorID == 0))
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Info("CTPTD:query trade success!");
			}
		}
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:query trade failed. Reason:%s", pRspInfo->ErrorMsg);
			}
		}
		char cReqType = THOST_RT_ASYNC;
		ReqTypeMap::iterator iter = m_mapReqType.find(nRequestID);
		...
		{
			InitializeOrder();
			m_pTraderHandlerSpi->OnRtnInitializeFinished(THOST_API_TYPE_CTP);
		}
	}
}

void CThostTraderHandler::OnRspOrderInsert(CThostFtdcInputOrderField *pInputOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	string OrderRefKey;
	...
	else
	{
		if (m_pLogHandler != NULL)
		{
			m_pLogHandler->Error("CTPTD:OnRspOrderInsert");
		}
	}
}

void CThostTraderHandler::OnRspOrderAction(CThostFtdcInputOrderActionField *pInputOrderAction, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	else
	{
		if (m_pLogHandler != NULL)
		{
			m_pLogHandler->Error("CTPTD:OnRspOrderAction");
		}
	}
}

void CThostTraderHandler::OnRtnOrder(CThostFtdcOrderField *pOrder)
{
	string OrderRefKey;
 	...
 	if (THOST_FTDC_OST_Canceled == pOrder->OrderStatus || THOST_FTDC_OST_NoTradeNotQueueing == pOrder->OrderStatus || THOST_FTDC_OST_PartTradedNotQueueing == pOrder->OrderStatus)
	{
		...
		else
		{
			if (m_pLogHandler != NULL)
			{
				m_pLogHandler->Error("CTPTD:OnRtnOrder-Cancel");
			}
		}
	}
}

void CThostTraderHandler::OnRtnTrade(CThostFtdcTradeField *pTrade)
{
	string ExchangeOrderKey;
	...
	else
	{
		if (m_pLogHandler != NULL)
		{
			m_pLogHandler->Error("CTPTD:OnRtnTrade");
		}
	}
}

void CThostTraderHandler::OnRtnInstrumentStatus(CThostFtdcInstrumentStatusField *pInstrumentStatus)
{
...
int CThostTraderHandler::GetInstrument(CThostInstrumentField &InstrumentInfo, string InstrumentID)
{
...
int CThostTraderHandler::GetAllInstrument(vector<CThostInstrumentField> &vecInstrumentInfo)
{
	for (InstrumentInfoMap::iterator iter = m_mapInstrumentInfo.begin(); iter != m_mapInstrumentInfo.end(); ++iter)
	{
...
int CThostTraderHandler::GetInstrumentMarginRate(CThostInstrumentMarginRateField &InstrumentMarginRate, string InstrumentID)
{
...
int CThostTraderHandler::GetInstrumentCommissionRate(CThostInstrumentCommissionRateField &InstrumentCommissionRate, string InstrumentID)
{
...
int CThostTraderHandler::GetTradingAccount(CThostTradingAccountField &TradingAccount)
{
...
int CThostTraderHandler::GetInvestorPosition(InvestorPositionVec &vecInvestorPosition, string InstrumentID)
{
...
int CThostTraderHandler::GetOrder(OrderReportVec &vecOrderReport, string InstrumentID, string ExchangeID, string OrderSysID, string InsertTimeStart, string InsertTimeEnd)
{
...
int CThostTraderHandler::GetTrade(TradeVec &vecTrade, string InstrumentID, string ExchangeID, string TradeID, string TradeTimeStart, string TradeTimeEnd)
{
	m_vecTrade.clear();
...
string CThostTraderHandler::GetInvestorID()
{
	return m_sInvestorId;
}

int CThostTraderHandler::GetTradingDay()
{
	return m_nTradingDay;
}

void CThostTraderHandler::MakeOrderRefKey(string &OrderRefKey, int FrontId, int SessionId, const char *pszOrderRef)
{
...
void CThostTraderHandler::MakeExchangeOrderKey(string &ExchangeOrderKey, const char *pszExchangeId, const char *pszOrderSysId)
{
...
CThostOrderReportField* CThostTraderHandler::MakeOrderReport(CThostFtdcInputOrderField *pInputOrder)
{
...
CThostOrderReportField* CThostTraderHandler::MakeOrderReport(CThostFtdcOrderField *pOrder)
{
	CThostOrderReportField *pOrderReport = new CThostOrderReportField();
	memset(pOrderReport, 0, sizeof(CThostOrderReportField));
...
void CThostTraderHandler::InitializeOrder()
{
	...
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Info("CTPTD:Init order success!");
	}
}
