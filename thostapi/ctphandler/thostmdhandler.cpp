#include "thostmdhandler.h"

CThostMdHandler::CThostMdHandler()
{
	m_pMdApi = NULL;
	m_pLogHandler = NULL;
}

CThostMdHandler::CThostMdHandler(string InvestorId, string Password, string BrokerId, string FrontAddress, string UserDataPath):
	CThostMdApi(InvestorId, Password, BrokerId, FrontAddress, UserDataPath)
{
	m_pMdApi = NULL;
	m_pLogHandler = NULL;
}

int CThostMdHandler::Start(string JsonParam)
{
	return ReqConnectFront();
}

int CThostMdHandler::Wait()
{
	m_pMdApi->Join();
	return 0;
}

void CThostMdHandler::Stop()
{
	m_pMdApi->Release();
}

int CThostMdHandler::ReqConnectFront()
{
	string sflowpath;
	if (m_sUserDataPath != "")
	{
		sflowpath = m_sUserDataPath + "/flow/ctp/md/" + m_sInvestorId + "/";
	}
	else
	{
		string sflowpath = "./flow/ctp/md/" + m_sInvestorId + "/";
	}
	...
	if (nRet)
	{
		...
		{
			m_pLogHandler->Info("CTPMD:front connect success!");
		}
	}
	else
	{
		if (m_pLogHandler != NULL)
		{
			m_pLogHandler->Error("CTPMD:front connect failed!");
		}
	}
...
void CThostMdHandler::OnFrontConnected()
{
	m_bConnected = true;
	...
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Info("CTPMD:OnFrontConnected");
	}
}	

void CThostMdHandler::OnFrontDisconnected(int nReason)
{
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Error("CTPMD:OnFrontDisconnected:%d",nReason);
	}
}


void CThostMdHandler::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
...
void CThostMdHandler::OnRspSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (m_pLogHandler != NULL)
	{
		m_pLogHandler->Info("CTPMD:%s subscribe success!", pSpecificInstrument->InstrumentID);
	}
...
void CThostMdHandler::OnRspUnSubMarketData(CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	vector<string>::iterator iter = m_vecSubscribeList.begin();
	for (; iter != m_vecSubscribeList.end(); ++iter)
	{
...
void CThostMdHandler::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData)
{
...
void CThostMdHandler::SubscribeQuote(char **ppInstrument, int count)
{
